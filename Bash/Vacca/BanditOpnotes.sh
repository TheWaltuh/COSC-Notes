#!/bin/bash

Level 1 password - NH2SXQwcBdpmTEzi3bvBHMM9H66vVXjL

Level 2 password - rRGizSaX8Mk1RTb1CNQoXTcYZWU6lgzi

Level 3 password - aBZ0W5EmUfAf7kHTQeOwd8bauFJ2lAiG

Level 4 password - 2EW7BBsr6aMMoJ2HjW067dm8EgX26xNe

Level 5 password - lrIWWI6bB37kxfiCQZqUdOIYfr6eEeqR

#find $HOME -type f -size 1033c ! -executable -exec file {} + | grep ASCII
Level 6 password - P4L4vucdmLnm8I7Vl7jG1ApGSfjYKqJU

find / -type f -size 33c -group bandit6 -user bandit7 2>/dev/null
Level 7 password - z7WtoNQU2XfjmMtWA8u5rN4vzqu4v99S

Level 8 password - TESKZC0XvTetK0S9xNwm25STk5iWrBvP

Level 9 password - EN632PlfYiZbn3PhVK3XOGSlNInNE00t

Level 10 password - G7w8LIi6J3kTb8A7j9LgrywtEUlyyp6s

Level 11 password - 6zPeziLdR2RKNdNYFNb6nVCKzphlXHBM

Level 12 password - JVNBBFSmZwKKOP0XbFXOoW8chDz5yVRv 

xxd command will reverse a hexdump
cat data.txt | xxd -r > data
file data (to find out information about the file which in this case is gzip compressed)
mv data data2.gz#rename the file to .gz extension
gzip -d data2.gz to decompress it
file data2 (again to find info in this case bzip2)
mv data2 data3.bz
bzip2 -d data3.bz
...
...
mv data4 data5.tar
tar -xf data5.tar
mv data5.bin data6.tar
tar -xf data6.tar
Level 13 password - wbWdlBxEir4CaE8LaPhauuOo6pwRmrDw

Level 14 password - 
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
chmod 600 for a private key
ssh -i (specifys a private key instead of a password)
fGrHPx402xGC7U7rXKDaxiWFTOiF0ENq

To send a message to a specific port:
echo "message" | nc localhost 30000
Level 15 password - jN2kgmIXJ6fShzhT2avhotn4Zcka6tnt

Send a message to a specific port but using SSL encryption:
echo "password" | openssl s_client -connect localhost:30001 -ign_eof
Level 16 password - JQttfApK4SeyHwDlI9SXGR50qclOAil1 

nmap -p 31000-32000 localhost
PORT      STATE SERVICE
31046/tcp open  unknown
31518/tcp open  unknown
31691/tcp open  unknown
31790/tcp open  unknown
31960/tcp open  unknown
echo "password" | openssl s_client -connect localhost:eachport -ign_eof

Level 17 password - f9wS9ZUDvZoo3PooHgYuuWdawDFvGld2

diff passwords.new passwords.old
Level 18 password - hga5tuuCLF6fFzUpnagiMN8ssu9LFrdg

Level 19 password - awhqfNnAbc1naukrpqDYcF95h7HoMTrC

Level 20 password - VxCazJaVykI6W36BkBU0mJTCM8rR95XT

using TMUX to multiplex:
tmux
(-n to prevent newline?)
echo -n "password" | nc -l 1234
ctrl+b+"
./suconnect 1234
Level 21 password - NvEJF7oVjkddltPSrdKEFOllh9V1IBcq

Level 22 password - WdDozAdTM2z9DiFEQ2mGlwngMfj4EZff

Level 23 password - QYw0Y2aiA672PsMmh9puTQuhoz8SyR2G

